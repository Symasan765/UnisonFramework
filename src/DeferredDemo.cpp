...
void cDeferredDemo::SetDeferredRendering(ID3D11ShaderResourceView* _DepthMap)
{
...
ID3D11ShaderResourceView * cDeferredDemo::GetResourceView(int _no)
{
	return m_pRenderTergetTex[_no].GetTextureResourceView();
}

GBuffer cDeferredDemo::GetGraphicBuffer()
{
...
void cDeferredDemo::SetDeferredShader()
{
	m_Shader->Set();
}
...
cDeferredDemo::cDeferredDemo()
{
	...
	//シェーダデータロード
	m_Shader = SHADER::GetInstance()->LoadShaderFile("DeferredDemo.hlsl", inPOSITION | inNORMAL | inTEX_UV | inBone | inWeight, true);
}
